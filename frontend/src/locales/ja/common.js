...
const TRANSLATIONS = {
  "onboarding": {
    ...
    "llm": {
      "title": "LLMの設定",
      "description": "PrivateGPTは多くのLLMプロバイダーと連携できます。これがチャットを処理するサービスになります。"
    },
    "userSetup": {
      "title": "ユーザー設定",
    ...
    "survey": {
      "title": "PrivateGPTへようこそ",
      "description": "PrivateGPTをあなたのニーズに合わせて構築するためにご協力ください。任意です。",
      "email": "メールアドレスは何ですか？",
      "useCase": "PrivateGPTを何に使用しますか？",
      "useCaseWork": "仕事用",
      "useCasePersonal": "個人用",
      "useCaseOther": "その他",
      "comment": "PrivateGPTをどのように知りましたか？",
  ...
  "common": {
    ...
    "no": "いいえ",
    "search": "検索",
    "username_requirements": "ユーザー名は2〜64文字で、小文字で始まり、小文字、数字、アンダースコア、ハイフン、ピリオドのみを含む必要があります。",
    "on": "～について",
    "none": "なし",
    "stopped": "停止",
    "loading": "読み込み中",
  ...
  "agent": {
    ...
    "skill": {
      ...
      "scheduledJob": {
        "title": "計画されたタスクを作成する",
        "description": "エージェントがチャットから繰り返し実行されるタスク（例：「毎日午前9時に、私のインボックスとメールを要約してメールで通知する」）を作成できるようにします。この機能はシングルユーザーモードでのみ利用可能です。"
      }
    },
    "mcp": {
      "title": "MCP サーバー",
    ...
    "settings": {
      ...
      "intelligent-skill-selection": {
        "title": "知的なスキル選択",
        "description": "クエリごとに、無制限のツールを使用し、トークン使用量を最大80%削減できます。PrivateGPTは、各プロンプトに対して最適なスキルを自動的に選択します。",
        "max-tools": {
          "title": "マックスツールズ",
  ...
  "api": {
    "title": "APIキー",
    "description": "APIキーにより、プログラム経由でこのPrivateGPTインスタンスにアクセスおよび管理できます。",
    "link": "APIドキュメントを読む",
    ...
    "modal": {
      ...
      "close": "閉じる",
      "create": "APIキーを作成",
      "helper": "作成したAPIキーは、このPrivateGPTインスタンスにプログラムからアクセスして設定するために使用できます。",
      "name": {
        "label": "名前",
        "placeholder": "本番環境の統合",
  ...
  "llm": {
    "title": "LLMの設定",
    "description": "これは、お好みのLLMチャットおよび埋め込みプロバイダー用の認証情報と設定です。これらのキーが最新かつ正確でない場合、PrivateGPTは正しく動作しません。",
    "provider": "LLMプロバイダー",
    "providers": {
      "azure_openai": {
  ...
  "transcription": {
    ...
    "warn-start": "RAMやCPUが限られたマシンでローカルのWhisperモデルを使用すると、メディアファイルの処理中にPrivateGPTが停止する可能性があります。",
  ...
  "embedding": {
    ...
    "desc-end": "埋め込みとは、テキストをベクトルに変換するプロセスです。これらの認証情報は、ファイルやプロンプトをPrivateGPTが処理できるフォーマットに変換するために必要です。",
    "provider": {
      "title": "埋め込みプロバイダー"
    }
  },
  ...
  "vector": {
    "title": "ベクターデータベース設定",
    "description": "これは、PrivateGPTインスタンスの動作方法用の認証情報と設定です。これらのキーが最新で正確であることが重要です。",
    "provider": {
  ...
  "privacy": {
    "title": "プライバシーとデータ処理",
    "description": "これは、接続されているサードパーティプロバイダーとPrivateGPTがデータをどのように処理するかの設定です。",
  ...
  "connectors": {
    ...
    "upload": {
      ...
      "privacy-notice": "これらのファイルは、このPrivateGPTインスタンス上のドキュメント処理機能にアップロードされます。第三者に送信・共有されることはありません。"
    },
    "pinning": {
      "what_pinning": "ドキュメントのピン留めとは？",
      "pin_explained_block1": "PrivateGPTでドキュメントを<b>ピン留め</b>すると、その内容全体がプロンプトウィンドウに挿入され、LLMがしっかり理解できるようになります。",
    ...
    "watching": {
      ...
      "watch_explained_block1": "PrivateGPTでドキュメントを<b>ウォッチ</b>すると、元のソースから定期的に内容が<i>自動的に</i>同期されます。管理しているすべてのワークスペースで内容が自動更新されます。",
    ...
    "gitea": {
      "name": "ギテアのリポジトリ",
      "description": "Gitea の任意のインスタンスから、公開またはプライベートなリポジトリ全体を 1 つのクリックでインポートします。",
      "URL": "ギテアのリポジトリURL",
      "URL_explained": "収集したいリポジトリのGiteaインスタンス上のURL – 自社ホストのリポジトリもサポートされています。",
      "token": "ギテア アクセス トークン",
      "optional": "（オプション）",
      "token_explained": "プライベートリポジトリや、認証が必要なインスタンス上のリポジトリを取得するには、アクセストークンが必要です。",
      "token_explained_start": "～なしで",
      "token_explained_link1": "アクセス トークン",
      "token_explained_end": "ただし、Gitea インスタンスが公開しているリポジトリのみを収集できます。",
      "ignores": "ファイルは無視する",
      "git_ignore": "`.gitignore`形式で、収集時に特定のファイルを無視するためのリストを作成します。保存したい項目ごとにEnterキーを押してください。",
      "task_explained": "すべてのファイルが完了すると、ドキュメントピッカーを使用してワークスペースに埋め込むことができます。",
      "branch": "ファイルを収集したいブランチを指定してください。",
      "branch_loading": "— 利用可能なブランチのロード中 —",
      "branch_explained": "ファイルを収集したいブランチの名前。",
      "token_information": "<b>Giteaアクセストークン</b>を入力しない場合、このデータコネクタは、あなたのGiteaインスタンス上の公開で読み取り可能なリポジトリからのみファイルを収集できます。"
    }
  },
  "chat_window": {
    ...
    "memories": {
      ...
      "modal": {
        ...
        "label": "記憶",
        "placeholder": "例：ユーザー名がジョー、ユーザーが使用しているツールがPrivateGPTなど。",
        "create": "作成する",
        "save": "保存",
        "cancel": "キャンセル"
      }
  ...
  "customization": {
    "interface": {
      "title": "UI設定",
      "description": "PrivateGPT の UI 設定を調整してください。"
    },
    "branding": {
      "title": "ブランディングとホワイトレーベル化",
      "description": "PrivateGPTインスタンスを、独自のブランドでカスタマイズしてください。"
    },
    "chat": {
      "title": "チャット",
      "description": "PrivateGPT のチャット設定をカスタマイズしてください。",
      "auto_submit": {
        "title": "自動音声入力送信",
    ...
    "items": {
      ...
      "display-language": {
        "title": "表示言語",
        "description": "PrivateGPTのUIを特定の言語で表示するためのオプションを選択してください。翻訳が利用可能な場合にのみ有効です。"
      },
      "logo": {
        "title": "ブランドロゴ",
      ...
      "disable-auto-scroll": {
        ...
        "description": "新しいメッセージを受信した際に、チャットを自動的に最後までスクロールする機能を停止します。"
      }
    }
  },
  "main-page": {
  ...
  "community_hub": {
    "publish": {
      ...
      "generic": {
        "unauthenticated": {
          "title": "本人確認が必要です。",
          "description": "アイテムを公開する前に、PrivateGPTコミュニティハブで認証する必要があります。",
          "button": "コミュニティハブへの接続"
        }
      },
      "slash_command": {
  ...
  "security": {
    ...
    "password": {
      "title": "パスワード保護",
      "description": "PrivateGPTインスタンスをパスワードで保護します。これを忘れた場合、回復方法はないため、このパスワードを必ず保存してください。",
  ...
  "imageGeneration": {
    "title": "画像生成の好み",
    "description": "`/img` コマンドを使用して画像を生成するために使用するプロバイダーを設定します。",
    ...
    "pending": {
      "heading": "画像の生成中…",
      "description": "これには少し時間がかかる場合があります。準備が整ったら、こちらに表示されます。",
