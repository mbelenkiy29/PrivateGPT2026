...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      title: "LLM 偏好",
      description:
        "PrivateGPT 可以与多家 LLM 提供商合作。这将是处理聊天的服务。",
    },
    userSetup: {
      title: "用户设置",
  ...
  agent: {
    ...
    settings: {
      ...
      "intelligent-skill-selection": {
        title: "智能技能选择",
        description:
          "实现无限工具和按查询减少高达 80% 的 Token 使用量——PrivateGPT 能够自动选择最合适的技能，以应对每个提示。",
        "max-tools": {
          title: "麦克斯工具",
          description:
  ...
  llm: {
    title: "LLM 首选项",
    description:
      "这些是你首选的 LLM 聊天和嵌入提供商的凭据和设置。重要的是，确保这些密钥是最新的和正确的，否则 PrivateGPT 将无法正常运行。",
    provider: "LLM 提供商",
    providers: {
      azure_openai: {
  ...
  transcription: {
    ...
    provider: "转录提供商",
    "warn-start":
      "在 RAM 或 CPU 有限的计算机上使用本地耳语模型可能会在处理媒体文件时停止 PrivateGPT。",
  ...
  embedding: {
    ...
    "desc-end":
      "嵌入是将文本转换为矢量的过程。需要这些凭据才能将你的文件和提示转换为 PrivateGPT 可以用来处理的格式。",
    provider: {
      title: "嵌入引擎提供商",
    },
  },
  ...
  vector: {
    title: "向量数据库",
    description:
      "这些是 PrivateGPT 实例如何运行的凭据和设置。重要的是，这些密钥是最新的和正确的。",
    provider: {
      title: "向量数据库提供商",
  ...
  connectors: {
    ...
    upload: {
      ...
      "fetch-website": "获取网站",
      "privacy-notice":
        "这些文件将被上传到此 PrivateGPT 实例上的文档处理器。这些文件不会发送或共享给第三方。",
    },
    pinning: {
      what_pinning: "什么是文档固定？",
      pin_explained_block1:
        "当您在 PrivateGPT 中<b>固定</b>一个文档时，我们会将整个文档内容注入到您的提示窗口中，让 LLM 能够完全理解它。",
      pin_explained_block2:
        "这在 <b>大上下文模型</b> 或关键的小文件中效果最佳。",
      pin_explained_block3:
        "如果默认情况下无法从 PrivateGPT 获取满意的答案，固定文档是提高答案质量的好方法。",
      accept: "好的，知道了",
    },
    watching: {
      what_watching: "什么是监控文档？",
      watch_explained_block1:
        "当您在 PrivateGPT 中<b>监控</b>一个文档时，我们会<i>自动</i>按定期间隔从其原始来源同步文档内容。系统会自动更新在所有使用该文档的工作区中的内容。",
    ...
    publish: {
      ...
      generic: {
        unauthenticated: {
          title: "需要验证",
          description:
            "在发布项目之前，您需要通过 PrivateGPT 社区中心进行验证。",
          button: "连接到社区中心",
        },
      },
      slash_command: {
  ...
  security: {
    ...
    password: {
      title: "密码保护",
      description:
        "用密码保护你的PrivateGPT实例。如果你忘记了密码，那么没有恢复方法，所以请确保保存这个密码。",
      "password-label": "实例密码",
    },
  },
  home: {
  ...
  telegram: {
    title: "Telegram 机器人",
    description:
      "将您的 PrivateGPT 实例与 Telegram 连接起来，这样您就可以从任何设备与您的工作空间进行聊天。",
    setup: {
      step1: {
